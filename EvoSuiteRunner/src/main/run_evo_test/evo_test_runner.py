...
def get_test_classes(project_name: str, method_name: str):
    test_classes = []
    test_dir =  f"{EVO_TEST_DIR}/{project_name}/classes/{method_name}"
...
def run_evo_tests(project_name: str, project_class_path: str, method_name: str):
    
    test_classes = get_test_classes(project_name, method_name)
    ...
    for test_class in test_classes:
        command = (
            ...
            f"{EVO_TEST_DIR}/{project_name}/classes/{method_name}:{LIB_PATH}/hamcrest-2.2.jar org.junit.runner.JUnitCore {test_class}"
        )
        
        # テストの実行
        os.system(command)
        generate_coverage_report(project_name, project_class_path, method_name)


# Jacoco.execをもとにカバレッジレポートを生成
def generate_coverage_report(project_name: str, project_class_path: str, method_name: str):
    ...
    command = (
        ...
        f"{project_class_path} --sourcefiles {project_src_path} --html {EVO_TEST_DIR}/{project_name}/report/{method_name}"
    )
    # カバレッジレポート生成の実行
